export const N_SQUARED_PLUS_N_CUBED_THETA_N_CUBED = {
  ...
  blocks: [
    ...
    { id: 'block1-2', latex: 'f(n) = {{complexity}}(g(n)) \\text{ if there exist positive constants } c_1 \\text{ and } n_0 \\text{ such that } 0 {{op}} f(n) {{op}} c_1g(n) \\text{ for all } n {{op}} n_0.' },
    { id: 'block1-3', latex: 'f(n) = {{complexity}}(g(n)) \\text{ if there exist positive constants } c_2 \\text{ and } n_0 \\text{ such that } 0 {{op}} c_2g(n) {{op}} f(n) \\text{ for all } n {{op}} n_0.' },
    { id: 'block1-4', latex: '\\text{Consider } f(n) = n^2 + n^3 \\text{ and } g(n) = n^3.' },
    { id: 'block1-5', latex: '\\text{For O-notation: } n^2 + n^3 {{op}} n^3 + n^3 \\text{ (for } n {{op}} 1 \\text{)}' }, // Added \text{} around "for" and "1"
    { id: 'block1-6', latex: 'n^2 + n^3 {{op}} 2n^3' }, // Purely math, should be fine
    ...
    { id: 'block1-11', latex: '\\text{Therefore, } n^2 + n^3 = {{complexity}}(n^3).' },
  ],
...
export const LOG_N_IS_O_N = {
  ...
  statement: '\\log n = O(n)',
  blocks: [
    { id: 'block2-1', latex: '\\text{To show } f(n) = {{complexity}}(g(n)) \\text{, we need to find positive constants } c \\text{ and } n_0 \\text{ such that } 0 {{op}} f(n) {{op}} cg(n) \\text{ for all } n {{op}} n_0.' },
    { id: 'block2-2', latex: '\\text{Consider } f(n) = \\log n \\text{ and } g(n) = n.' },
    { id: 'block2-3', latex: '\\text{We know that } \\log n {{op}} n \\text{ for all } n {{op}} 1.' },
